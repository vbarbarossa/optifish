...
# check if we are close to optimality
tcheck <- foreach(i = c(100,1000,10000,50000,100000),.combine = 'rbind') %do% {
  
  t1 <- readRDS(paste0('~/surfdrive/tmp/optifish_proc_20220830/nsga2_allDams_ic_ci_gen',i,'_pop100.rds'))
  t1 <- -t1@fitness %>% as.data.frame()
  t1$no <- as.character(format(i, scientific = TRUE))
  return(t1)
}
colnames(tcheck)[1:2] <- c('ic','ci')
tcheck$no <- factor(tcheck$no, levels = as.character(format(c(100,1000,10000,50000,100000), scientific = TRUE)))

p <- ggplot(data = tcheck,
            aes(x = ic, y = ci, color = no)) +
  # geom_line(alpha = 1, size = 1.5) +
  geom_point() +
  # geom_step(direction = 'hv') +
  # geom_smooth(se = F) +
  scale_color_manual(values = RColorBrewer::brewer.pal(9,'YlOrRd')[c(4,5,6,7,9)]) +
  # labs(color = '') +
...
# check for future dams
tcheck <- foreach(i = c(10000,50000,100000),.combine = 'rbind') %do% {
  
  t1 <- readRDS(paste0('~/surfdrive/tmp/optifish_proc_20220830/nsga2_futureDams_ic_ci_gen',i,'_pop100.rds'))
  t1 <- -t1@fitness %>% as.data.frame()
  t1$no <- as.character(format(i, scientific = TRUE))
  return(t1)
}
colnames(tcheck)[1:2] <- c('ic','ci')
tcheck$no <- factor(tcheck$no, levels = as.character(format(c(10000,50000,100000), scientific = TRUE)))

p <- ggplot(data = tcheck,
            aes(x = ic, y = ci, color = no)) +
  geom_line(alpha = 1, size = 1.5) +
  # geom_smooth(se = F) +
  scale_color_manual(values = RColorBrewer::brewer.pal(9,'YlOrRd')[c(4,7,9)]) +
  # labs(color = '') +
  xlab('Installed Capacity [MW]') +
  ylab('Connectivity Index [%]') +
  scale_x_continuous(labels = function(x) format(x, scientific = TRUE)) +
...
dams_data <- dams[,c('INTER_ID','Status','DamHeight','ReferenceY',name_col_IC, name_col_V)]
...
op <- foreach(i = c('all','future'),.combine = 'rbind') %do% {
  
  t1 <- readRDS(paste0('~/surfdrive/tmp/optifish_proc_20220830/nsga2_',i,'Dams_ic_ci_gen50000_pop100.rds'))
  t1 <- -t1@fitness %>% as.data.frame()
  t1$scenario <- i
  return(t1)
}
...
op$scenario <- factor(op$scenario, levels = c('all','present','future'))
op$ci <- op$ci*100
p <- ggplot(data = op,
  ...
  scale_color_manual(values = RColorBrewer::brewer.pal(8,'Dark2')[c(1,2,3)]) +
  labs(color = '') +
  xlab('Installed Capacity [MW]') +
  ylab('Connectivity Index [%]') +
  scale_x_continuous(labels = function(x) format(x, scientific = TRUE)) +
...
ggsave('figs/pareto_ic_ci_noyr.jpg',p,width = 150, height = 120, units = 'mm',dpi = 600)
