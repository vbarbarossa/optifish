...
# do the full calculation per species
# (CI <- lapply(split(sp_data_inter,sp_data_inter$binomial), function(x){
#   bas_sp <- x %>%
...
#   bas_tot <- bas_sp
#   bas_tot$pass <- 1
#   bas_tot$pass[bas_tot$INTER_ID %in% rbind(dams_e,dams_f)$INTER_ID] <- 0
...
#   g_pass <- graph_from_data_frame(d=df)
#   # plot(g_pass)
#   
#   E(g_pass)$pass <- bas_tot$pass
#   V(g_pass)$names <- bas_tot$INTER_ID
#   vertices_id <- names(igraph::V(g_pass))
#   C <- 10^dodgr::dodgr_dists(
#     mutate(
#       rbind( 
#         igraph::as_data_frame(g_pass, what = "edges"), 
#         igraph::as_data_frame(as.undirected(g_pass,mode = 'each'), what = "edges")
#       ),dist = log10(.data$pass)
#     ),
#     from = vertices_id, to = vertices_id)
#   
#   return(sum(C*L,na.rm=T))
# }))

# # try for a single species
# bas_sp <- sp_data_inter %>%
#   filter(binomial == sp) %>%
#   select(INTER_ID,l = L_tot) %>%
#   right_join(inter_basins %>% select(INTER_ID,INTER_NEXT),by = "INTER_ID")
# bas_sp$l[is.na(bas_sp$l)] <- 0
...
bas_tot$pass <- 1
# bas_tot$pass[bas_tot$INTER_ID %in% rbind(dams_e,dams_f)$INTER_ID] <- 0
bas_tot$pass <- sample(seq(0,0.7,0.1),nrow(bas_tot),replace = T)
df <- data.frame(from = bas_tot$INTER_ID,to = bas_tot$INTER_NEXT)
outlet <- as.character(df$from[which(df$to == 0)])
...
sum(C*L,na.rm=T)/sum(L)

# # the same, but takes double the time
# gr <- g_pass
# E(gr)$pass_u <- E(gr)$pass_d <- E(gr)$pass
# C1 = riverconn::c_ij_fun(gr,dir_fragmentation_type = 'asymmetric')
# sum(C1*L)/sum(L)
...
dams_data <- dams[,c('INTER_ID','Status',name_col_IC, name_col_V)]
...
CI_df <- t(as.data.frame(CI)) %>% as.data.frame
CI_df$binomial <- row.names(CI_df)
colnames(CI_df)[1] <- "CI_new" 

names(CI_alt) <- spp
CI_dfo <- t(as.data.frame(CI_alt)) %>% as.data.frame
CI_dfo$binomial <- row.names(CI_dfo)
colnames(CI_dfo)[1] <- "CI_old" 
CI_dfo$CI_old <- CI_dfo$CI_old/100

CI_df <- left_join(CI_df,CI_dfo)

CI_df$CI_new <- round(CI_df$CI_new,4)
CI_df$CI_old <- round(CI_df$CI_old,4)
CI_df$equal <- (CI_df$CI_new == CI_df$CI_old)
# Run the algorithm -------------------------------------------------------------------------
