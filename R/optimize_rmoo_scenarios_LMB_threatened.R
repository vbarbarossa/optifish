...
source('R/prepare_data_LMB_threatened.R')
# ------------------------------------------------------------------------------
...
for(scen in c('pristine')){ #'pristine_pass_bin','pristine_pass_step','pristine_pass_lin'
  ...
  cat('\nSaving..')
  saveRDS(op,paste0('proc/LMBthreatened_nsga2_',scen,'_',paste(save_str,collapse = '_'),'_gen',gen_size,'_pop',pop_size,'.rds'))
...
for(s in 1:2){ #'mitigation_step','mitigation_lin'
  ...
  # pareto pristine
  save_str <- c('ic','vol','sed','ci')[c(energy,water,sedimentation,fragmentation)]
  pp <- readRDS(paste0('proc/LMBthreatened_nsga2_pristine_',paste(save_str,collapse = '_'),'_gen',gen_size,'_pop',pop_size,'.rds'))
