...
# filter out china
china_borders <- rnaturalearth::ne_countries(scale = 10, country = 'China', returnclass = 'sf') %>%
  select(name)

# based on percentage overlap
hb_data$area_ <- st_area(hb_data)
inter <- st_intersection(hb_data,china_borders) %>%
  mutate(new_area = st_area(.))

#calculate ratio overlap
inter$area_ratio <- units::drop_units(inter$new_area/inter$area_)

# # visual check
# st_write(hb_data %>% filter(HYBAS_ID %in% inter$HYBAS_ID[inter$area_ratio > 0.75]),'hb_overlap75.gpkg')
# st_write(hb_data %>% filter(HYBAS_ID %in% inter$HYBAS_ID[inter$area_ratio > 0.5]),'hb_overlap50.gpkg')

# exclude basins with >50% overlap (after visual check)
hb_data <- hb_data %>% 
  filter(!HYBAS_ID %in% inter$HYBAS_ID[inter$area_ratio > 0.5])
...
iucn_file <- 'proc/iucn_file.csv'
if(!file.exists(iucn_file)){
  ...
  write.csv(iucn_code,iucn_file,row.names = F)
}else{
  iucn_code <- read.csv(iucn_file)
}

# 674 of 724 species have a match for IUCN status
sum(iucn_code$binomial %in% unique(sp_data$binomial))

# 66 are listed at threatened (threat level >= VU)
sum(iucn_code$binomial[iucn_code$code %in% c('VU','EN','CR')] %in% unique(sp_data$binomial))

sp_data <- sp_data %>% filter(binomial %in% iucn_code$binomial[iucn_code$code %in% c('VU','EN','CR')])
# ------------------------------------------------------------------------------
