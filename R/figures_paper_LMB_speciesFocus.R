...
for(runs_set in c('','40sp','threatened')){
  ...
  op <- foreach(i = c('pristine','future'),.combine = 'rbind') %do% {
  ...
  # op <- foreach(i = c('LMB40sp_nsga2_pristine','LMB40sp_nsga2_future','LMB40threatened_nsga2_pristine','LMB40threatened_nsga2_future'),.combine = 'rbind') %do% {
  #   
  #   t1 <- readRDS(paste0('proc/',i,'_ic_ci_gen',gen_size,'_pop100.rds'))
  #   t1 <- -t1@fitness %>% as.data.frame()
  #   t1$scenario <- i
  #   return(t1)
  # }
  ...
  if(runs_set != ''){
    write.csv(op,paste0('proc/',runs_set,'_pareto_tab.csv'),row.names = F)
  }else{
    write.csv(op,paste0('proc/',runs_set,'pareto_tab.csv'),row.names = F)
  }
...
# create one table
op <- bind_rows(read.csv('proc/pareto_tab.csv'),
                read.csv('proc/40sp_pareto_tab.csv'),
                read.csv('proc/threatened_pareto_tab.csv'))
op$runs_set[is.na(op$runs_set)] <- 'all'
op_plot <- op
op_plot$scenario <- factor(op_plot$scenario, levels = c('pristine','present','future'))
...
p_pareto <- ggplot() +
  ...
  geom_point(data = op_plot %>% filter(runs_set == 'All species'),
             aes(x = ic, y = ci, shape = runs_set, 
                 fill = scenario),alpha = 0.5, size = 2) +
  geom_point(data = op_plot %>% filter(runs_set != 'All species'),
             aes(x = ic, y = ci, shape = runs_set, 
                 fill = scenario),alpha = 1, size = 2) +
...
ggsave('figs/fig5.pdf',p_pareto,
       width = 100, height = 100, units = 'mm', scale = 1.2)

# alternatively a 2 panels
op_plot$runs_set <- as.character(op_plot$runs_set)
shapes = c(21,21)
op$ic <- op$ic/1000
p_pareto <- ggplot() +
  # geom_step(data = op_plot %>% filter(scenario == c('existing')),direction = 'hv', show.legend = F, linetype = 2) +
  geom_point(data = op_plot %>% filter(runs_set == 'All species') %>% select(ic,ci),
             aes(x = ic, y = ci),fill = 'grey80',shape = 21,alpha = 0.5, size = 2, color = 'grey80') +
  geom_point(data = op_plot %>% filter(runs_set != 'All species'),
             aes(x = ic, y = ci, shape = runs_set, 
                 fill = scenario),alpha = 1, size = 2) +
  scale_fill_manual(values = fills) +
...
ggsave('figs/fig5_alt.pdf',p_pareto,
       width = 180, height = 100, units = 'mm', scale = 1)
